...
//
// The list of reorderable slots: every `defineRenderSlot` (those whose `.Render`
// applies the reorder middleware) and every `defineOrderedDispatchSlot` (dispatch
// slots with a structurally-required contribution `id`); mount slots are excluded.
// Each entry's `pluginId` is the *defining* plugin's id (its tree path), so reorder
