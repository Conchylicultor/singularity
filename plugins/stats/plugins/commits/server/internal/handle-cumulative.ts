...
export const handleLinesCumulative = implement(
  getCommitsLinesCumulative,
  async ({ query }) => {
      ...
      const points = days.map((date) => {
        ...
        return {
          ...
          byExt: Object.fromEntries(
            Object.entries(running).map(([k, v]) => [k, { ...v }]),
          ),
        };
      });
      return { points };
