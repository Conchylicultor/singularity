...
export const handleCumulative = implement(
  getCostCumulative,
  async ({ query }) => {
    const scope: Scope = query.scope ?? "singularity";
    const { daily, projectIsSingularity } = await loadBundle();
...
export const handleDailyByFamily = implement(
  getCostDailyByFamily,
  async ({ query }) => {
    const scope: Scope = query.scope ?? "singularity";
    ...
    for (const s of filtered) {
      ...
      const usedFamilies = new Set(
        s.modelsUsed.map((m) => modelFamily(canonicalModel(m))),
      );
...
export const handleDistribution = implement(
  getCostDistribution,
  async ({ query }) => {
    const scope: Scope = query.scope ?? "singularity";
    const { sessions } = await loadBundle();
    return {
      buckets: logCostBuckets(
        filterSessions(sessions, scope).map((s) => s.cost),
      ),
    };
  },
);
...
export const handleAvgPerConversation = implement(
  getCostAvgPerConversation,
  async ({ query }) => {
    const scope: Scope = query.scope ?? "singularity";
    const { sessions } = await loadBundle();
