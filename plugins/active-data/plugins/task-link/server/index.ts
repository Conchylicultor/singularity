...
export default {
  description:
    "The task-id token at the page-editor's server boundary: locates `task-<id>` spans and names the shared active-data inline node, so a page block holding one of these chips stays agent-readable and agent-editable. Declares itself markdown-TRANSPARENT — a bare id has no character the inline scan could misread.",
  contributions: [
    ...
    // While the two were one field, `` `task-<id>` `` pasted as markdown lost its
