...
`notifCooldownMs: 6h`, like `render-loop` — a raise above the engine's 10-minute re-alert floor
(a cooldown is only ever a raise; there is no "never"). A still-present divergence re-fires on
every edit of the affected surface (each op latches its own stalled report, but new ops on a
broken surface keep filing onto the same fingerprint), so without the raise the bell would
re-alert every 10 minutes for one known problem.
...
- Web:
  - Contributes:
    - `Core.Root` → `OptimisticDivergenceCollector`
    - `Reports.KindView` → `OptimisticDivergenceKindView`
  - Uses:
    - `primitives/css/badge.Badge`
    - `primitives/css/inline.Inline`
    - `primitives/optimistic-mutation.optimisticDivergenceReportSink`
    - `reports.report`
    - `reports.Reports`
- Server:
...
- Core:
  - Exports (types): `OptimisticDivergencePayload`
  - Exports (values):
