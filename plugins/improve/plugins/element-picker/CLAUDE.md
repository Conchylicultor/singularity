...
anchored CSS path) — is captured, serialized to a single-line `<ui-context …/>`
tag, and handed to the Improve popover via
...
`UI_CONTEXT_RE`. The tag is `<ui-context plugin="…" slot="…" pane="…" path="…"
url="…" element="…" selector="…" />` (plugin/slot/pane/path/selector omitted when
absent; `path` carries the outer→inner lineage `plugin@Slot > plugin@Slot` and is
only emitted when it adds more than the innermost plugin/slot). Attribute values
are sanitized to be quote/newline-free so the tag stays single-line and
round-trippable.
