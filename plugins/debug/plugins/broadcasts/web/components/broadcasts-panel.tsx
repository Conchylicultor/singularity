...
export function BroadcastsPanel() {
  ...
  const save = useCallback(
    async (updated: BroadcastEntry[]): Promise<boolean> => {
      setSaving(true);
      setError(null);
      try {
  ...
  return (
    <Stack gap="none" className="h-full">
      {/* Header */}
      <Line className="border-b px-lg py-md">
        <Fill>
        ...
        <Stack gap="sm" className="border-b bg-muted/30 px-lg py-md">
          ...
          <Stack direction="row" gap="sm" align="center">
            <Text
              as="span"
              variant="caption"
              className={cn("w-16 text-muted-foreground", rigidClass())}
            >
              Severity
            </Text>
            <Stack direction="row" gap="xs">
              {(["error", "warning", "info"] as BroadcastSeverity[]).map(
                (s) => (
                  <ToggleChip
                    key={s}
                    variant="ghost"
                    ...
                    className={
                      form.severity === s ? SEVERITY_STYLES[s] : undefined
                    }
                  >
                    {s}
                  </ToggleChip>
          ...
          <Stack direction="row" gap="sm">
            <Text
              ...
              className={cn("mt-1.5 w-16 text-muted-foreground", rigidClass())}
            >
              Message
            </Text>
            <textarea
            ...
            <Stack direction="row" gap="sm" align="center">
              ...
              {form.commands.length === 0 && (
                <Text
                  ...
                  className="ml-1 italic text-muted-foreground"
                >
                  (all)
                </Text>
              )}
            ...
            {entries.map((entry, i) => (
              ...
              >
                <Badge
                  ...
                  className={cn("mt-0.5", rigidClass())}
                >
                  {entry.severity}
                </Badge>
                <Fill>
                  ...
                  {(entry.since ?? entry.until ?? entry.commands) && (
                    <Stack
                      ...
                      className="mt-1 font-mono text-3xs text-muted-foreground"
                    >
                      {entry.since && (
                        <span>since: {entry.since.slice(0, 8)}</span>
                      )}
                      {entry.until && (
                        <span>until: {entry.until.slice(0, 8)}</span>
                      )}
                      {entry.commands && (
