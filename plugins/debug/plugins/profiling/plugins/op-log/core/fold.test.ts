...
import { foldLegacyBuildRecords, foldLegacyPushRecords } from "./internal/legacy";
import type { RawLegacyBuildRecord, RawLegacyPushRecord } from "./internal/legacy";
...
describe("foldLegacyPushRecords", () => {
  const legacyTerminal: RawLegacyPushRecord = {
    phase: "completed",
    pushId: "p-1",
    opSlug: "wt-a",
    branch: "feature",
    ...
    mode: "worktree",
    startedAt: at(0),
    lockRequestedAt: at(1_000),
    lockAcquiredAt: at(4_000),
    completedAt: at(64_000),
    preLockMs: 1_000,
    waitMs: 3_000,
    holdMs: 60_000,
    totalMs: 64_000,
    outcome: "success",
    steps: [{ name: "rebase", startMs: 0, durationMs: 500 }],
  };

  test("maps a terminal push onto a single push-mutex wait", () => {
    const [rec] = foldLegacyPushRecords([legacyTerminal], T0);
    expect(rec!.kind).toBe("push");
    expect(rec!.opId).toBe("p-1");
    expect(rec!.waits).toEqual([{ kind: "push-mutex", startMs: 0, durationMs: 3_000 }]);
    expect(rec!.waitMs).toBe(3_000);
    expect(rec!.holdMs).toBe(60_000);
    expect(rec!.totalMs).toBe(64_000);
    expect(rec!.outcome).toBe("success");
    expect(rec!.mode).toBe("worktree");
    expect(rec!.steps).toHaveLength(1);
    // The legacy writer never recorded a lane.
    expect(rec!.lane).toBeNull();
  });

  test("a no-phase legacy line is terminal", () => {
    const { phase: _phase, ...noPhase } = legacyTerminal;
    const [rec] = foldLegacyPushRecords([noPhase], T0);
    expect(rec!.outcome).toBe("success");
    expect(rec!.totalMs).toBe(64_000);
  });

  test("lock_requested only → waiting, growing against now", () => {
    const raw: RawLegacyPushRecord[] = [
      { phase: "lock_requested", pushId: "p-2", opSlug: "wt-b", branch: "b", startedAt: at(0), lockRequestedAt: at(0) },
    ];
    const [rec] = foldLegacyPushRecords(raw, T0 + 20_000);
    expect(rec!.outcome).toBe("waiting");
    expect(rec!.waits).toEqual([{ kind: "push-mutex", startMs: 0, durationMs: 20_000 }]);
    expect(rec!.totalMs).toBe(20_000);
  });

  test("lock_requested + lock_acquired → running, wait computed across the two lines", () => {
    const raw: RawLegacyPushRecord[] = [
      { phase: "lock_requested", pushId: "p-3", branch: "b", startedAt: at(0), lockRequestedAt: at(0) },
      { phase: "lock_acquired", pushId: "p-3", lockAcquiredAt: at(5_000) },
    ];
    const [rec] = foldLegacyPushRecords(raw, T0 + 65_000);
    expect(rec!.outcome).toBe("running");
    expect(rec!.waits).toEqual([{ kind: "push-mutex", startMs: 0, durationMs: 5_000 }]);
    expect(rec!.holdMs).toBe(60_000);
  });

  test("an interrupted legacy terminal keeps its flag", () => {
    const [rec] = foldLegacyPushRecords(
      [{ ...legacyTerminal, completedAt: null, outcome: "error", interrupted: true, holdMs: 0, totalMs: 0 }],
      T0,
    );
    expect(rec!.interrupted).toBe(true);
    expect(rec!.totalMs).toBe(0);
...
describe("foldLegacyBuildRecords", () => {
  test("the oldest format — NO phase field at all — is terminal with no waits", () => {
    // Verified on disk: build-log.jsonl's oldest lines are exactly this shape.
    const raw: RawLegacyBuildRecord[] = [
      {
        worktree: "wt-a",
        branch: "main",
        startedAt: at(0),
        completedAt: at(944_764),
        totalMs: 944_764,
        success: true,
      },
    ];
    const [rec] = foldLegacyBuildRecords(raw);
    expect(rec!.kind).toBe("build");
    expect(rec!.waits).toEqual([]);
    expect(rec!.waitMs).toBe(0);
    expect(rec!.holdMs).toBe(944_764);
    expect(rec!.totalMs).toBe(944_764);
    expect(rec!.outcome).toBe("success");
    expect(rec!.interrupted).toBe(false);
    expect(rec!.buildId).toBeNull();
    // For a build the log's `worktree` IS the op slug.
    expect(rec!.opSlug).toBe("wt-a");
  });

  test("a started/completed pair merges on (worktree, startedAt)", () => {
    const raw: RawLegacyBuildRecord[] = [
      { phase: "started", worktree: "wt-a", branch: "f", buildId: "b1", startedAt: at(0), completedAt: null, totalMs: 0, success: false },
      { phase: "completed", worktree: "wt-a", branch: "f", buildId: "b1", startedAt: at(0), completedAt: at(30_000), totalMs: 30_000, success: true },
    ];
    const recs = foldLegacyBuildRecords(raw);
    expect(recs).toHaveLength(1);
    expect(recs[0]!.outcome).toBe("success");
    expect(recs[0]!.buildId).toBe("b1");
    expect(recs[0]!.opId).toBe(`wt-a:${at(0)}`);
  });

  test("an unpaired started is an interrupted marker with no duration", () => {
    const raw: RawLegacyBuildRecord[] = [
      { phase: "started", worktree: "wt-a", branch: "f", startedAt: at(0), completedAt: null, totalMs: 0, success: false },
    ];
    const [rec] = foldLegacyBuildRecords(raw);
    expect(rec!.interrupted).toBe(true);
    expect(rec!.outcome).toBe("error");
  ...
  test("a genuine build failure is `failed`, not `error` — it has a real duration", () => {
    const raw: RawLegacyBuildRecord[] = [
      { phase: "completed", worktree: "wt-a", branch: "f", startedAt: at(0), completedAt: at(9_000), totalMs: 9_000, success: false },
    ];
    const [rec] = foldLegacyBuildRecords(raw);
    expect(rec!.outcome).toBe("failed");
    expect(rec!.interrupted).toBe(false);
    expect(rec!.totalMs).toBe(9_000);
  });

  test("two worktrees' builds interleaved in one file stay separate", () => {
    const raw: RawLegacyBuildRecord[] = [
      { phase: "started", worktree: "wt-a", branch: "a", startedAt: at(0), completedAt: null, totalMs: 0, success: false },
      { phase: "started", worktree: "wt-b", branch: "b", startedAt: at(0), completedAt: null, totalMs: 0, success: false },
      { phase: "completed", worktree: "wt-b", branch: "b", startedAt: at(0), completedAt: at(1_000), totalMs: 1_000, success: true },
    ];
    const byId = new Map(foldLegacyBuildRecords(raw).map((r) => [r.opId, r]));
    expect(byId.size).toBe(2);
    expect(byId.get(`wt-b:${at(0)}`)!.outcome).toBe("success");
    expect(byId.get(`wt-a:${at(0)}`)!.interrupted).toBe(true);
  });
});
