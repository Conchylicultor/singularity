...
- **Host-gate load.** `loadConcurrency > 0` saturates the host-wide `heavy-read` gate
  via `load-generator.ts`: occupants run on the LIVE `heavy-read` pool handle
  (`defineHostPool` is a per-id registry, so occupying `"heavy-read"` returns the very
  pool `host-read-pool` defined), so they hold the IDENTICAL flock slots and force the
  measured burst onto the broker wait path. Async holders
...
Uses (hand-written, beyond the autogen list below): `infra/host/host-read-pool`
(`withHeavyReadSlot` / `heavyReadSlotCount`), `runtime-profiler/core.waitSplit`,
`infra/host/host-admission.defineHostPool` (occupies the live `heavy-read` pool).
