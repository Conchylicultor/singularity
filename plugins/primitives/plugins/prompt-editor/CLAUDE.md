...
The action row under the text is padded by the density group's
`padComposerActions` (`sm` sides and an `xs` foot by default), so a theme that
pads the whole box (`padComposer`) can drop the row's own inset.
