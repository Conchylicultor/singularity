# toggle-chip

## Passthrough: `ref` is the chip, not its label

A toggle chip IS a `Badge`, so `ref` and everything else you spread land where
Badge's do — the chip shell (a `<button>` by default), never the `truncate` span
inside it. `ref` is newly typed; it was already flowing untyped through the old
anonymous index signature. See
[the passthrough contract](../../../passthrough/CLAUDE.md).

<!-- AUTOGENERATED:BEGIN — do not edit; regenerated by `./singularity build` -->

## Plugin reference

- Description: Toggle-chip control: a stateful pill (composes Badge) in one of three colour treatments — solid (filled when on), ghost (accent fill when on, transparent when off) and tinted (accent wash when on, bordered when off) — with active state, button-height matching, polymorphic `as`, plus a SegmentedControl single-select group helper.
- Web:
  - Uses:
    - `primitives/css/badge.Badge`
    - `primitives/css/ui-kit.cn`
    - `primitives/css/ui-kit.ControlSize`
    - `primitives/css/ui-kit.DensityControlled`
    - `primitives/css/ui-kit.useControlSize`
  - Exports (types):
    - `SegmentedControlProps`
    - `SegmentedOption`
    - `ToggleChipProps`
    - `ToggleChipVariant`
  - Exports (values):
    - `SegmentedControl`
    - `ToggleChip`
- Cross-plugin:
  - Imported by:
    - `apps-core/surface`
    - `apps-core/surface/floating`
    - `apps-core/surface/floating/wallpaper`
    - `apps/chord/piano`
    - `apps/deploy/analytics/dashboard`
    - `apps/events/sources`
    - `apps/events/sources/source-detail/schedule`
    - `apps/prototypes/compare`
    - `apps/prototypes/gallery`
    - `apps/sonata/audio/metronome`
    - `apps/sonata/pedal/indicator`
    - `apps/sonata/piano-roll`
    - `apps/sonata/rich/chord-mode`
    - `apps/sonata/rich/chord-progression`
    - `apps/sonata/rich/chord-readout`
    - `apps/sonata/rich/key-readout`
    - `apps/sonata/rich/rhythm-controls`
    - `apps/sonata/rich/voicing-controls`
    - `apps/sonata/sources/ultimate-guitar`
    - `apps/sonata/view-options`
    - `apps/studio/compositions`
    - `apps/studio/compositions/contributors`
    - `apps/studio/compositions/release`
    - `apps/studio/graph`
    - `build/serve-composition`
    - `config_v2/fields`
    - `config_v2/settings`
    - `conversations/conversation-ui/chip`
    - `conversations/conversation-view/code/file-pane`
    - `debug/broadcasts`
    - `debug/queue`
    - `debug/timeline`
    - `fields/enum/inline`
    - `fields/tags/inline`
    - `page/inline-date`
    - `page/place`
    - `primitives/data-view`
    - `primitives/data-view/view-core`
    - `primitives/filter-chips`
    - `primitives/text-editor/composer`
    - `primitives/view-switcher`
    - `review`
    - `shell/notifications`
    - `stats`
    - `stats/commits`
    - `stats/cost`
    - `stats/pushes`
    - `stats/responsiveness`

<!-- AUTOGENERATED:END -->
