...
list of pre-rule offenders (471 grandfathered files) has been **fully drained to
0** — every former offender now composes through the primitives — so the rule
enforces repo-wide with only the permanent primitive exemption. Do not add
entries back. A genuinely-fixed one-off escapes per-site via
