...
set is itself a registry — the `inline-chip` sub-plugin's chips, which fill in
as the plugin tiers load. Registering a finished list there would freeze whatever had
...
- **`inline-chip`** — Inline chips (`att-…`, `<ui-context>`): `inlineChip()`
  declares one, and one generic node renders every declared chip in every
  `TextEditor` and on read surfaces. See its `CLAUDE.md`.
