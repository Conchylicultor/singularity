...
gated per row by `FieldDef.canEdit` like every other view (see
["Inline editing"](#inline-editing-onedit--oneditvalues--canedit)).
...
## Inline editing (`onEdit` / `onEditValues` / `canEdit`)

A field declares a write-back and becomes click-to-edit in **every** view:

```ts
onEdit?: (row: TRow, next: FieldValue) => void | Promise<void>;       // pairs with `value`
onEditValues?: (row: TRow, next: string[]) => void | Promise<void>;   // pairs with `values`
canEdit?: (row: TRow) => boolean;                                     // per-row gate
```

Declaring `onEdit`/`onEditValues` makes the field editable for **every** row: the
host resolves a per-type editor via the `data-view.cell-editor` slot and calls the
write-back on commit (the consumer owns persistence — data-view stays
presentational). Absent → the cell is read-only, the default for most fields.

`canEdit` **withdraws that capability per row**. It defaults to `true` (declaring a
write-back means editable); returning `false` renders that row's cell/label
**read-only — no editor and no inert affordance** (the gated row is
indistinguishable from a field that declares no write-back at all, so a user is
never offered an edit that would be discarded). Use it for **heterogeneous row
unions** where only some kinds are writable, and for read-only / archived rows.

It is the exact twin of `ManualOrderConfig.getRank` returning `null` ("this row is
not a drag source") — a per-row withdrawal, not a second capability. Like that one,
it is honored **uniformly**: the shared `FieldCell` → `EditableCell` path
(table / list / gallery, and the tree's secondary chips) and the tree's primary
label (`EditableTreeLabel`) apply the same gate, so it is never a per-view quirk.

```ts
{ id: "title", label: "Title", primary: true,
  value: (r) => r.title,
  onEdit: (r, next) => renameGroup(r.id, String(next ?? "")),
  // only `group` rows have a rename endpoint — conversations are read-only
  canEdit: (r) => r.kind === "group" }
```
...
sections.length === 1 && sections[0]!.key === null ? (
  ...
  <GroupedSections
    ...
    headerClassName="px-sm"                  // align the header to the body's inset
  >
...
groups, degrading to the swap hand-off above that** — in every view, for free. The
only per-view axis is `headerClassName` (the horizontal inset matching that view's
body padding).
...
- Cross-plugin:
<<<<<<< .merge_file_zNPzHk
  - Imported by: `apps/deploy/servers`, `apps/home/app-cards`, `apps/mail/inbox`, `apps/pages/page-tree`, `apps/prototypes/gallery`, `apps/sonata/library`, `apps/story/shell`, `apps/studio/compositions`, `apps/studio/compositions/closure-tree`, `apps/studio/compositions/release`, `apps/studio/explorer`, `apps/workflows/definitions`, `apps/workflows/executions`, `build`, `code-explorer`, `config_v2/settings`, `conversations/agents`, `conversations/all-conversations`, `conversations/conversations-view/data-view/history`, `conversations/conversations-view/data-view/queue`, `debug/boot-profile`, `debug/profiling/runtime`, `debug/reports`, `debug/slow-ops/cluster`, `debug/slow-ops/pane`, `debug/trace/pane`, `fields/bool/data-view-codec`, `fields/bool/filter`, `fields/bool/inline`, `fields/bool/table`, `fields/color/table`, `fields/date/data-view-codec`, `fields/date/filter`, `fields/date/inline`, `fields/date/table`, `fields/enum/column-config`, `fields/enum/filter`, `fields/enum/inline`, `fields/enum/table`, `fields/image/table`, `fields/number/data-view-codec`, `fields/number/filter`, `fields/number/inline`, `fields/number/table`, `fields/tags/filter`, `fields/tags/inline`, `fields/tags/table`, `fields/text/filter`, `fields/text/inline`, `fields/text/table`, `page/links`, `primitives/data-view/custom-columns`, `primitives/data-view/gallery`, `primitives/data-view/list`, `primitives/data-view/server-query`, `primitives/data-view/table`, `primitives/data-view/tree`, `primitives/data-view/view-order`, `tasks/task-deps-tree`, `tasks/task-list`, `ui/tweakcn/community-browser`
=======
  - Imported by: `apps/deploy/servers`, `apps/home/app-cards`, `apps/mail/inbox`, `apps/pages/page-tree`, `apps/prototypes/gallery`, `apps/sonata/library`, `apps/story/shell`, `apps/studio/compositions`, `apps/studio/explorer`, `apps/studio/release`, `apps/workflows/definitions`, `apps/workflows/executions`, `build`, `code-explorer`, `config_v2/settings`, `conversations/agents`, `conversations/all-conversations`, `conversations/conversations-view/data-view/grouped`, `conversations/conversations-view/data-view/history`, `conversations/conversations-view/data-view/queue`, `debug/boot-profile`, `debug/profiling/runtime`, `debug/reports`, `debug/slow-ops/cluster`, `debug/slow-ops/pane`, `debug/trace/pane`, `fields/bool/data-view-codec`, `fields/bool/filter`, `fields/bool/inline`, `fields/bool/table`, `fields/color/table`, `fields/date/data-view-codec`, `fields/date/filter`, `fields/date/inline`, `fields/date/table`, `fields/enum/column-config`, `fields/enum/filter`, `fields/enum/inline`, `fields/enum/table`, `fields/image/table`, `fields/number/data-view-codec`, `fields/number/filter`, `fields/number/inline`, `fields/number/table`, `fields/tags/filter`, `fields/tags/inline`, `fields/tags/table`, `fields/text/filter`, `fields/text/inline`, `fields/text/table`, `page/links`, `primitives/data-view/custom-columns`, `primitives/data-view/gallery`, `primitives/data-view/list`, `primitives/data-view/server-query`, `primitives/data-view/table`, `primitives/data-view/tree`, `primitives/data-view/view-order`, `tasks/task-deps-tree`, `tasks/task-list`, `ui/tweakcn/community-browser`
>>>>>>> .merge_file_XFax4X
- Core:
