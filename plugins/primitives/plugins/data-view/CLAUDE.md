...
Flat views window large lists through the shared `<VirtualRows>` component
(`web/components/virtual-rows.tsx`, barrel-exported) so a multi-thousand-row data
source stays cheap to render and scroll. It wraps `@tanstack/react-virtual` with
dynamic row measurement (variable heights supported) behind a small API:
`items`, `estimateSize`, `getKey`, `itemClassName?`, `overscan?`, plus a
...
Recent tab is the worked example). Only the **list** view virtualizes today
(threshold 100 rows; below that the plain `.map` runs unchanged); table, gallery,
and tree are follow-ups (see `research/2026-06-18-data-view-row-virtualization.md`).
