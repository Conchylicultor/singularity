...
(falling back to nearest `[data-pane-id]` → `<main>` → scrolling root). While the
gesture is held, each wasted event accumulates signed `raw` intent; the surface's
inline `transform: translate(±Npx)` is set to a **damped** offset — `MAX_PULL *
(1 − MAX_PULL / (MAX_PULL + |raw|·RESISTANCE))` — which approaches `MAX_PULL`
(~48px) asymptotically, so pushing harder yields ever-smaller gains (down/right →
nudge content up/left, like native). No transition is applied during the hold, so
it tracks the gesture frame-by-frame.
...
driven from inline styles in the framework-free detector — the plugin ships no
CSS.
