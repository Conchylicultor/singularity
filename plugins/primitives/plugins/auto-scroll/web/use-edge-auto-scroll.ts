...
function velocityFor(
  ...
  if (fromBottom <= fromTop) {
    return fromBottom >= threshold ? 0 : speedFraction(fromBottom, threshold) * maxSpeed;
  }
  return fromTop >= threshold ? 0 : -speedFraction(fromTop, threshold) * maxSpeed;
}
...
export function useEdgeAutoScroll({
  ...
  const startLoop = useEventCallback(() => {
    ...
    function step(time: number): void {
      ...
      const dt =
        Math.min(time - (lastTimeRef.current ?? time), MAX_FRAME_DELTA_MS) / 1000;
