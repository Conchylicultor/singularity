...
- `surfaceUndoProps` — a region whose edits are recorded on THIS stack, so a
  ...
  no per-block Lexical history, and every field in it (block text, the code
  block's textarea, an embed's URL) records through the same chokepoints.
- `localUndoProps` — an editor keeping its own history, claiming the key back
  from an enclosing surface region. Only nesting needs it.

**Undeclared resolves off the target**: a text-editing host is `local`, anything
else `surface`. So the marker is opt-in and fails safe — a missing one costs a
keystroke that stays in the field being typed in, never a document rewound behind
the user's back. That test is narrower than `shortcuts`' `isEditableTarget` on
