...
import { attachmentsDir } from "@plugins/infra/plugins/attachments/data-dirs";
...
export async function assembleAttachments(
  ...
  if (!existsSync(attachmentsDir.path)) {
    return {
      id: "attachments",
      name: "Attachments",
      skipped: false,
  ...
  await cp(attachmentsDir.path, dir, { recursive: true });
