...
import { COST_USAGE_DIR } from "@plugins/infra/plugins/paths/server";
import type { BackupSourceItem, BackupSourceReport } from "@plugins/backup/core";
import { costHistorySourceConfig } from "../../shared/config";

export async function assembleCostHistory(dir: string): Promise<BackupSourceReport> {
  const { enabled } = getConfig(costHistorySourceConfig);

  if (!enabled) {
    return { id: "cost-history", name: "Cost History", skipped: true, items: [], sizeBytes: 0 };
  }
  ...
  let shards = 0;
  if (existsSync(COST_USAGE_DIR)) {
    for await (const rel of new Bun.Glob("sessions-*.json").scan({
      cwd: COST_USAGE_DIR,
      onlyFiles: true,
    })) {
      const dest = join(dir, rel);
      await cp(join(COST_USAGE_DIR, rel), dest);
  ...
  items.push({ label: "sessions", detail: `${shards} year shards`, count: shards });
  ...
  const priceTablePath = join(COST_USAGE_DIR, "price-table.json");
  ...
  return { id: "cost-history", name: "Cost History", skipped: false, items, sizeBytes };
}
