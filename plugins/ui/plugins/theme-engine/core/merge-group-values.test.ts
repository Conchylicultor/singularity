...
it("fills dark-mode holes from a field's darkDefault, light-mode holes from its default", () => {
  const result = mergeGroupValues(
    {
      background: { default: "white", darkDefault: "black" },
      radius: { default: "0.5rem" },
    },
    { light: {}, dark: {} },
    {},
  );
  expect(result.light).toEqual({ background: "white", radius: "0.5rem" });
  // A field with no darkDefault is mode-independent: dark reuses `default`.
  expect(result.dark).toEqual({ background: "black", radius: "0.5rem" });
});
