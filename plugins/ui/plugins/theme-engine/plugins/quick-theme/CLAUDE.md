...
its contributors — `theme-engine/theme-gallery` — also contributes to the
...
1. `QuickTheme.Section` — contributed quick sections: the theme picker
   (`theme-engine/theme-gallery`), then the light/dark switch
   (`ui/theme-toggle`). Order is reorder config's
   (`config/ui/theme-engine/quick-theme/section.jsonc`), not this file's.
2. `ThemeEngine.VariantGroup` — every component variant picker (sidebar
   framing, tab bar, progress bar, window titlebar). Each is a choice that
   survives a theme swap; token values are never picked here, only through the
