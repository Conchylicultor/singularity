...
import { fontFamilyGroup } from "./group";

const tokenSubFields = Object.fromEntries(
  Object.entries(fontFamilyGroup.schema).map(
    ([key, { label }]) => [key, textField({ default: "", label })]
  )
);

export const fontFamilyConfig = defineConfig({
  scope: "app",
  fields: {
    preset: dynamicEnumField({ default: "default", label: "Font preset" }),
