...
For the same reason there is **no full inverse**. The pair is recorded where it
is minted (`composition.json` carries `composition` + `checkout`), so "which
composition is this?" is answered from provenance, which cannot be ambiguous.
`namespaceParts(ns)` reads back only what the name itself fixes: a two-label
name (`sonata.att-X`) names both halves, and a single label comes back as a
`label` the caller must interpret — never silently as "no checkout".
