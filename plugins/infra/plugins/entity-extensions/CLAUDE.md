...
Lets sub-plugins attach typed DB fields to a parent plugin's entity table without coupling the parent. Mirrors `attachments.defineLink` but for 1:1 side-tables.
...
export const agentAutoLaunchExt = defineExtension(_agents, "auto_launch", {
  enabled: boolean("enabled").notNull().default(false),
});

await agentAutoLaunchExt.upsert(agentId, { enabled: true });
const row = await agentAutoLaunchExt.get(agentId);
```
