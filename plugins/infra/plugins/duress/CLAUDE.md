...
- `server/internal/latch.ts` — the whole latch (state, fs, memos) + its
  co-located bun test. Tests point the latch at a temp dir via
  `_setLatchDirForTests` and drive the memo/lease math via `_setClockForTests`
  (internal-only escape hatches, not on the barrel).
...
- Server:
  - Uses: `config_v2.ConfigV2`, `config_v2.getConfig`, `infra/paths.SINGULARITY_DIR`, `primitives/log-channels.Log`, `primitives/log-channels.LogChannel`
  - Exports: Types: `DuressLatch`, `ShedBuffer`, `ShedBufferOptions`, `ShedCascadeStats`, `ShedSummary`; Values: `clearDuress`, `createShedBuffer`, `duressEpisode`, `FRESHNESS_LEASE_MS`, `isUnderDuress`, `LATCH_FILENAME`, `MEMO_TTL_MS`, `readDuress`, `refreshDuress`, `setDuress`
- Core:
  - Uses: `config_v2.defineConfig`, `fields/bool/config.boolField`, `fields/int/config.intField`
  - Exports: Values: `duressConfig`
- Cross-plugin:
  - Imported by: `debug/sentinel`, `debug/slow-ops`, `debug/trace/engine`, `reports`
