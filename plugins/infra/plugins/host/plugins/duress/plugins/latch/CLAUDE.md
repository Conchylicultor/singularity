...
- Server:
  ...
  - Exports (values):
    ...
    - `setDuress`
- Test helpers:
  - Server: `@plugins/infra/plugins/host/plugins/duress/plugins/latch/server/testing`
    - `_setClockForTests` — Override the clock (memo TTL + freshness math).
    - `_setLatchDirForTests` — Point the latch at a temp dir.
