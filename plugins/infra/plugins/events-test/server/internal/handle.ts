...
export const handleSubscribe = implement(
  subscribeEventsTest,
  async ({ body }) => {
...
export const handleDirectEnqueue = implement(
  directEnqueueEventsTest,
  async ({ body }) => {
...
// Poll graphile_worker.jobs until no jobs on ANY of the plugin's task
// identifiers (one per hold class, plus the legacy one) are pending. Lets e2e
// tests synchronize between emit()/enqueue() (which now return once jobs are
// durable, not once handlers finish) and /log.
...
export async function handleWaitIdle(req: Request): Promise<Response> {
  ...
  return retryUntil(
    ...
    },
    {
      ...
      deadline: timeoutMs,
      onDeadline: () =>
        Response.json({ idle: false, timeoutMs }, { status: 408 }),
    },
  );
}
...
export const handleDeleteTargeting = implement(
  deleteEventsTestTargeting,
  async ({ body }) => {
...
export const handleListTriggers = implement(
  listEventsTestTriggers,
  async () => {
    const rows = await db
      .select()
