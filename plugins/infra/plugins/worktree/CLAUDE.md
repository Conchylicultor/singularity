...
instead of re-created. A failed add also claims its path on the removal seam, so the
removal audit does not report git's own rollback as an outside deletion.
