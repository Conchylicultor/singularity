...
The config-render capability — the `stringListField` factory plus the
drag-and-drop renderer (one input row per string, reorderable) contributed to
the frozen `config-v2.fields.renderer` slot — lives in the `plugins/config`
sub-plugin.
