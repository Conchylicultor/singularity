...
an explicit `id` is seeded by the config_v2 registry (`normalizeCollectionItems`),
at any nesting depth, as `auto-<hash([index, content])>` — idempotent per read but
**content- and position-dependent**; only the generic Settings-pane "Add item" UI
mints a real `crypto.randomUUID()`.
