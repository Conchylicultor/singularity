...
describe("tags filter-sql", () => {
  ...
  test("contains → jsonb containment of a one-element array", () => {
    expect(q(ops.contains(col, "INBOX"))).toEqual({
      sql: '"t"."c" @> $1::jsonb',
      params: ['["INBOX"]'],
    });
  });
  ...
  test("empty / non-string scalar operand → undefined (rule dropped)", () => {
    ...
    expect(ops.contains(col, 3)).toBeUndefined();
    expect(ops["does-not-contain"](col, "")).toBeUndefined();
  });

  test("empty / non-list operand → undefined (rule dropped)", () => {
    expect(ops["contains-any-of"](col, [])).toBeUndefined();
    expect(ops["contains-any-of"](col, "x")).toBeUndefined();
    expect(ops["contains-all-of"](col, [])).toBeUndefined();
    // a list with no string members is just as incomplete
    expect(ops["contains-all-of"](col, [1, 2])).toBeUndefined();
  });

  test("does-not-contain KEEPS null rows (JS reads a missing value as [])", () => {
    const r = q(ops["does-not-contain"](col, "SPAM"));
  ...
  test("contains-any-of → OR fold, one containment per tag", () => {
    const r = q(ops["contains-any-of"](col, ["a", 2, "b"]));
  ...
  test("contains-any-of with a single tag → same shape as contains", () => {
    expect(q(ops["contains-any-of"](col, ["a"]))?.params).toEqual(['["a"]']);
    expect(q(ops["contains-any-of"](col, ["a"]))?.sql).toBe(
      '"t"."c" @> $1::jsonb',
    );
  });

  test("contains-all-of → ONE containment carrying the whole list", () => {
    const r = q(ops["contains-all-of"](col, ["a", "b"]));
  ...
  test("operands are bound as params, never spliced", () => {
    const r = q(ops.contains(col, `'); DROP TABLE t; --`));
    expect(r?.sql).toBe('"t"."c" @> $1::jsonb');
