...
describe("date filter-sql (half-open day ranges)", () => {
  ...
  test("is-before → col < day", () => {
    expect(q(ops["is-before"](target, jan15))).toEqual({
  ...
  test("is-after → col >= nextDay", () => {
    expect(q(ops["is-after"](target, jan15))?.params).toEqual([day16]);
  });

  test("is-on-or-before → col < nextDay", () => {
    expect(q(ops["is-on-or-before"](target, jan15))?.params).toEqual([day16]);
  });

  test("is-on-or-after → col >= day", () => {
    expect(q(ops["is-on-or-after"](target, jan15))?.params).toEqual([day15]);
  });

  test("empty / invalid operand → undefined (incomplete rule)", () => {
    expect(ops.is(target, null)).toBeUndefined();
    expect(ops.is(target, "")).toBeUndefined();
    expect(ops.is(target, "not-a-date")).toBeUndefined();
    expect(ops["is-on-or-after"](target, undefined)).toBeUndefined();
  });
  ...
  test("is-between → [from, nextDay(to)); inclusive-to whole day", () => {
    ...
    const r = q(ops["is-between"](target, { from, to }));
  ...
  test("is-between open bounds", () => {
    const from = { kind: "date", iso: "2026-01-10" } as const;
    expect(q(ops["is-between"](target, { from }))?.sql).toBe(
      '"t"."c" >= to_timestamp($1 / 1000.0)',
    );
    expect(ops["is-between"](target, {})).toBeUndefined();
    expect(ops["is-between"](target, undefined)).toBeUndefined();
  });
  ...
  test("is-within-past → [lo, nextDay(hi)) around today", () => {
    ...
    const r = q(ops["is-within-past"](target, operand));
    expect(r?.sql).toBe(
      '("t"."c" >= to_timestamp($1 / 1000.0) AND "t"."c" < to_timestamp($2 / 1000.0))',
    );
  ...
  test("is-within-next → [lo, nextDay(hi))", () => {
    ...
    expect(q(ops["is-within-next"](target, operand))?.params).toEqual([
      lo,
      addUnits(hi, "day", 1),
    ]);
  });
  ...
  test("within: missing / non-positive amount → undefined", () => {
    expect(ops["is-within-past"](target, {})).toBeUndefined();
    expect(
      ops["is-within-past"](target, { unit: "day", amount: 0 }),
    ).toBeUndefined();
    expect(ops["is-within-next"](target, undefined)).toBeUndefined();
  });
