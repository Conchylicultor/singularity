...
The chip itself is not this plugin's. It is `ConversationChip`
(`conversations/conversation-ui/plugins/chip`) — the one shared "a conversation,
clickable, opening its run" widget, which also owns the `Starting…` fallback
title and the highlight while that run is the open column. This block owns only
the join and the order: oldest run first.
