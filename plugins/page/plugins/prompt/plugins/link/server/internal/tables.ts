...
// its own: the pk's implicit btree covers `parent_id` (the `taskId` key) and
// nothing else, so without one that read is a seq scan. `(block_id,
// created_at)` is the pair because that single index serves both reads the
// block-keyed resource makes — the FULL load filters on `block_id` and returns
// oldest-first, so an ordered index scan can satisfy the ORDER BY without a
// sort, and the scoped refill (`block_id = X AND parent_id IN (…)`) seeks on
// the same leading column. Which plan the planner actually picks is its call:
// while the table is small it may prefer a bitmap scan plus a sort, and the
// trailing column costs nothing.
