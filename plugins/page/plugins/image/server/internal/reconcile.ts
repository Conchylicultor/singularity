...
// Reconcile the block↔attachment link set for an entire page. For each image
// block, mirror its `attachmentId` (or clear it); for every other block, clear
// (a cheap no-op unless it was previously an image). `set()` is an idempotent
// insert-new/delete-removed reconcile.
export async function reconcilePageImages(pageId: string): Promise<void> {
