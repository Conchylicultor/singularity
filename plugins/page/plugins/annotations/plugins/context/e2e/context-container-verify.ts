...
//     a child of the SAME container, and NO second dashed box. The regression the
//     user reported — under the old model another content line meant another
//     container.
...
//
// - The two container types are told apart by their BACKDROP: the context card's
//   border is DASHED, the callout's tint has none. Not by any decoration selector
//   — the anchor's own control is this plugin's business, and a spec that named it
//   would fail for a cosmetic rename rather than for a regression.
...
interface FrameBox {
  ...
  right: number;
  height: number;
  /** A DASHED border identifies the context card's frame; the callout's tint has none. */
  dashed: boolean;
}
...
async function geometry(page: Page): Promise<Geometry> {
  return page.evaluate(() => {
    ...
    for (const child of grid ? [...grid.children] : []) {
      ...
      // painting. Take the first node in the subtree that actually paints —
      // a background OR a border, since the context card's box is defined by its
      // dashed border as much as by its tint.
...
/**
 * The frame box a given block OWNS, i.e. the one whose top edge sits on that
 * block's own row.
 *
 * Geometry rather than DOM position, and the top edge rather than containment,
 * because containment is ambiguous the moment containers nest: an enclosing
 * callout's tint also covers a nested card's rows. A frame always starts at its
 * container's own grid line, so the top edge identifies the owner — and `dashed`
 * separates a context card's box from a callout's tint in the one case where the
 * two tops coincide (both are anchors: a zero-height row, so a frame starts at the
 * same y as its first child's row).
 */
function boxOwnedBy(g: Geometry, blockId: string, dashed: boolean): FrameBox | undefined {
  const row = g.rows[blockId];
  if (!row) return undefined;
  return g.boxes.find((b) => b.dashed === dashed && Math.abs(b.top - row.top) <= 1);
}

/** The context card's dashed box for this block, if it paints one. */
function cardBox(g: Geometry, blockId: string): FrameBox | undefined {
  return boxOwnedBy(g, blockId, true);
}

/** The callout's (borderless) tint for this block, if it paints one. */
function tintBox(g: Geometry, blockId: string): FrameBox | undefined {
  return boxOwnedBy(g, blockId, false);
}

/** The earliest row in document order whose top sits on this box's top edge. */
function ownerOf(g: Geometry, box: FrameBox): string | undefined {
  return g.order.find((id) => {
    const row = g.rows[id];
    return row != null && Math.abs(row.top - box.top) <= 1;
  });
}

/**
 * The block ids of every row currently OWNING a dashed card box.
 *
 ...
 *
 * The owner is resolved as the FIRST row on the box's top edge: a void anchor row
 * is zero height and precedes its first child in document order, so the two share
 * a y and the anchor wins.
 */
function cardOwners(g: Geometry): string[] {
  return g.boxes
    .filter((b) => b.dashed)
    .map((b) => ownerOf(g, b))
...
function firstLineInside(g: Geometry, box: FrameBox): string | undefined {
  return g.order.find((id) => {
    const row = g.rows[id];
    return (
      row != null && row.height > 1 && row.top >= box.top - 1 && row.bottom <= box.bottom + 1
    );
  });
}
 ...
 */
async function probePost(page: Page, pageId: string, data: unknown): Promise<PostProbe> {
  return page.evaluate(
    async ({ parent, payload }) => {
      const res = await fetch("/api/blocks", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ parentId: parent, type: "context", data: payload }),
      });
...
async function seedCards(page: Page, pageId: string): Promise<SeedIds> {
  return page.evaluate(async ({ pageId: parent }) => {
    ...
    const context = (parentId: string) => post({ parentId, type: "context", data: {} });
    ...
    const unwrapSecond = await text(unwrapCard.id, "bulleted-list", "second inside");
...
await withBrowser(async (h) => {
  ...
  const withText = await probePost(page, pageId, { text: [{ text: "not allowed" }] });
  ...
  if (accepted) {
    ...
    if (probeId === undefined) {
      r.fail("void schema: the accepted probe returned a block row", withEmpty.body);
    } else {
  ...
  const ownersSeeded = cardOwners(g0);
  ...
  console.log("childless card:", JSON.stringify({ row: emptyRow0, box: emptyBox0 }));
  if (!emptyRow0) {
    ...
    const hit = await page.evaluate(
      ...
      { x: emptyRow0.contentLeft + 20, y: emptyRow0.top + emptyRow0.height / 2 },
    );
    r.ok(
  ...
  if (!box0) {
    r.fail(
      "heading-first: the card paints a dashed box owned by its own (zero-height) row",
      `boxes=${JSON.stringify(g0.boxes)} cardTop=${cardRow0.top}`,
    );
  } else {
    r.ok(
    ...
    r.ok(
      "heading-first: the box bleeds to the card's content edge, not past it",
      Math.abs(box0.left - cardRow0.contentLeft) <= 1,
      `box.left=${box0.left} contentEdge=${cardRow0.contentLeft}`,
    );
  }
  ...
  // containers are now anchors, so both boxes start on a zero-height row — the
  // dashed/borderless discriminator is what keeps them apart.
  ...
  r.ok(
    "nesting: BOTH the callout's tint and the nested card's dashed box are painted",
  ...
  if (!outerBox || !innerBox || !nestedRow0 || !calloutRow0) {
    ...
    r.ok(
      "nesting: the inner box is inset one indent deeper (each frame bleeds to its OWN content edge)",
      innerBox.left > outerBox.left,
      `outer.left=${outerBox.left} inner.left=${innerBox.left}`,
    );
    r.ok(
  ...
  r.ok(
    "seed: exactly the five seeded cards paint a box",
    ownersSeeded.length === 5,
    `owners=${JSON.stringify(ownersSeeded)}`,
  );
  r.ok(
    "seed: each dashed box is owned by its own card's row (no box orphaned onto a child)",
    [seeded.card, seeded.empty, seeded.enterCard, seeded.unwrapCard, seeded.nested].every(
      (id) => ownersSeeded.includes(id),
    ),
    `owners=${JSON.stringify(ownersSeeded)}`,
  );
  ...
  const ownersBeforeEnter = cardOwners(g0);
  ...
  if (!(await caretToEnd(page, seeded.enterChild))) {
    ...
    const gEnter = await geometry(page);
    const minted = gEnter.order.filter((id) => !orderBeforeEnter.includes(id));
    console.log("minted ids:", JSON.stringify(minted));
    r.ok("enter: exactly one new line was created", minted.length === 1, JSON.stringify(minted));
    ...
    if (newId === undefined) {
      ...
      r.ok(
        ...
        newRow != null && childRow != null && newRow.contentLeft === childRow.contentLeft,
        `child=${childRow?.contentLeft} new=${newRow?.contentLeft}`,
      );
      r.ok(
        "enter: it sits immediately after that child in document order",
        gEnter.order.indexOf(newId) === gEnter.order.indexOf(seeded.enterChild) + 1,
      ...
      const ownersAfterEnter = cardOwners(gEnter);
      const newCards = ownersAfterEnter.filter((id) => !ownersBeforeEnter.includes(id));
      r.ok(
        "enter: NO second card was minted (no new dashed box appeared)",
        newCards.length === 0,
        `new=${JSON.stringify(newCards)} owners=${JSON.stringify(ownersAfterEnter)}`,
      );
      r.ok(
        "enter: the new line is not itself a card (it owns no box)",
        cardBox(gEnter, newId) === undefined && !ownersAfterEnter.includes(newId),
      ...
      r.ok(
        ...
        grownBox != null && newRow != null && grownBox.bottom >= newRow.bottom - 1,
      ...
      const ownersBeforeUnwrap = cardOwners(gEnter);
      if (!(await caretToStart(page, seeded.unwrapFirst))) {
        r.fail("unwrap: could not place the caret at the start of the first child");
      } else {
        await page.keyboard.press("Backspace");
        await page.waitForTimeout(1800);
        await snap(page, out, "3-unwrapped");

        const gUnwrap = await geometry(page);
        r.ok(
        ...
        r.ok(
          "unwrap: its dashed box went with it, and no other card was disturbed",
          ownersBeforeUnwrap.includes(seeded.unwrapCard) &&
            !cardOwners(gUnwrap).includes(seeded.unwrapCard) &&
            ownersBeforeUnwrap
              .filter((id) => id !== seeded.unwrapCard)
              .every((id) => cardOwners(gUnwrap).includes(id)),
          `before=${JSON.stringify(ownersBeforeUnwrap)} after=${JSON.stringify(cardOwners(gUnwrap))}`,
        );
        r.ok(
        ...
        r.ok(
          ...
          endRow != null &&
            gUnwrap.rows[seeded.unwrapFirst]?.contentLeft === endRow.contentLeft &&
            gUnwrap.rows[seeded.unwrapSecond]?.contentLeft === endRow.contentLeft,
      ...
      const gBeforeWrap = await geometry(page);
      const ownersBeforeWrap = cardOwners(gBeforeWrap);
      const orderBeforeWrap = gBeforeWrap.order;
      if (!(await caretToStart(page, seeded.wrapMe))) {
        r.fail("wrap: could not place the caret at the start of the origin block");
      } else {
        ...
        const originEditable = page
          .locator(`[data-block-id="${seeded.wrapMe}"] [contenteditable="true"]`)
          .first();
        ...
        const gWrap = await geometry(page);
        const mintedCards = cardOwners(gWrap).filter((id) => !ownersBeforeWrap.includes(id));
        ...
        r.ok(
          ...
          caretBefore.anchorOffset != null &&
            ...
            (await caretState(originEditable)).anchorOffset === caretBefore.anchorOffset,
        ...
        const gB = await geometry(pageB);
        console.log("context B owners:", JSON.stringify(cardOwners(gB)));
        r.ok(
          "converge: the card's row is still zero-height with no editable of its own",
          gB.rows[seeded.card]?.height === 0 && gB.rows[seeded.card]?.editable === false,
        ...
        r.ok(
          ...
          JSON.stringify({ boxB, first: boxB ? firstLineInside(gB, boxB) : undefined }),
        );
        ...
        r.ok(
          ...
          anchorId != null &&
            cardOwners(gB).includes(anchorId) &&
        ...
        r.ok(
          ...
          gB.rows[seeded.unwrapFirst]?.contentLeft === gB.rows[seeded.end]?.contentLeft &&
            gB.rows[seeded.unwrapSecond]?.contentLeft === gB.rows[seeded.end]?.contentLeft,
          JSON.stringify({
