...
/**
 * The private-note card's leading glyph — the ONLY thing its row paints, and a
 ...
 * appearance `sections` — the payload is `{}`, so there is nothing per-instance
 * to configure and nothing for a popover to open onto. Its structural actions
 * (Collapse / Remove private note / Delete) come from the rail on the line it
 * borrows, generically over `BlockHandle.anchor`.
 *
 ...
 */
export function PrivateNotesAnchor() {
  return <ContainerAnchor glyph={<MdVisibilityOff className="size-5 text-destructive" />} />;
}
