 ...
 * over whole — the quote's own content box, so the rule stands exactly where the
 * prose above it starts), so this file declares nothing but the look. The
 * block's `data` is never read — a quote has no per-instance appearance.
 *
 * No tint and no box: a quote is the LIGHTEST container in the family (the
 * callout is a solid fill, the annotation cards a soft wash), and its one
 * identifying mark is the rule down its left edge.
 */
export function QuoteFrame(props: BlockFrameProps) {
  return (
    <ContainerBackdrop
      frame={props}
      className="border-muted-foreground/30 border-l-2"
    />
  );
}
