 ...
 *
 * Three cases, and the middle one is the reason the row is consulted at all:
 *
 ...
 *   that does not exist.
 *
 * `stalled` overrides both: the binding demonstrably renders less than its
 * replica holds, which is a real defect with a real remedy, so it gets a
 * VISIBLE retry (`rehydrate()` — end this session, start a new one) instead of
 * an indefinite skeleton. It is right-aligned so it never covers the start of
 * the line the user can still type into: `hydrating` gates the WRITE path, not
 * the editing host (see `collab-session.ts`), so the block stays live
 * throughout.
 *
...
export function HydrationPlaceholder({
  ...
  const rowLength = runsLength(runsOf((block.data as Record<string, unknown> | null)?.text));
  if (rowLength === 0) return null;
  const widthCh = Math.min(Math.max(rowLength, SKELETON_MIN_CH), SKELETON_MAX_CH);
  return (
    ...
    <Pin to="top-left" decorative aria-hidden className={TEXT_VARIANT_CLASS[textVariant]}>
      <Inset y="xs">
