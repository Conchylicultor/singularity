...
Two containers exist today — `page/callout` (a solid tint) and `page/context` (a
dashed box) — and this plugin is everything they have in common. It contributes
**nothing** itself: it is a library the container plugins build their own block
type out of.
...
`gutterFirstLineCenter` (the surface seats the decoration on the first child's
borrowed first line), and `markdownPrefixes` (from which a void type derives no
parser anyway).
