...
`PluginViewSlots.Section` is a
[`detail-sections`](../../../primitives/plugins/detail-sections/CLAUDE.md) slot —
the host owns the card and title, so a contribution is content-only. A section
with nothing to show declares `useAvailable` (a `return null` body still leaves
an empty titled card); a count goes in `summary`, via the shared `SectionCount`.
