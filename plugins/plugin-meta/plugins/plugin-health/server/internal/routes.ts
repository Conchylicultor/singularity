...
export const handleGetTasksForReview = implement(
  getPluginHealthTasks,
  async ({ params }) => {
    const { reviewId } = params;
    const rows = await db
      .select({
        taskId: healthReviewExt.table.taskId,
      ...
      .innerJoin(_tasks, eq(_tasks.id, healthReviewExt.table.taskId))
