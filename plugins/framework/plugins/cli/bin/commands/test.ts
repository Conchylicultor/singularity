 ...
 */
async function enumerateTests(root: string, target: string): Promise<{ rel: string; files: string[] }> {
  ...
  const files: string[] = [];
  for await (const hit of new Bun.Glob(TEST_FILE_GLOB).scan({ cwd: abs, onlyFiles: true })) {
...
async function runRunner(name: string, argv: string[], root: string): Promise<number> {
  console.log(`\n── ${name} ──`);
...
export function registerTest(program: Command) {
  program
    ...
    .argument("[paths...]", "Files or directories to test (default: the whole `plugins` tree)")
    .action(async (paths: string[]) => {
      ...
      const bunExit = bun.length > 0
        ? await runRunner("bun:test", [process.execPath, "test", ...runnerArgs], root)
        : null;
      ...
      const domExit = dom.length > 0
        ? await runRunner("vitest", [process.execPath, "x", "vitest", "run", ...runnerArgs], root)
        : null;
      ...
      const report = (label: string, exit: number | null, count: number, empty: string) =>
        console.log(
          exit === null
