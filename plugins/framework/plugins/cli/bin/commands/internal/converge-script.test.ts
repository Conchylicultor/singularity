...
import { PUT_HELPER_SH, RESTART_GATE_SH, convergeScript } from "./converge-script";
...
describe("convergeScript (idempotence contract)", () => {
  test("every generated file lands through put", () => {
    ...
    for (const target of ["$ENV_FILE", "$CADDY_LIST", "$SITE", "$CADDYFILE", "$UNIT_PATH"]) {
...
describe("the restart gate", () => {
  ...
  async function gate(opts: {
    ...
    writeFileSync(join(dir, "proc-stat"), `cpu 1 2 3\nbtime ${BTIME}\nprocesses 9\n`);
...
${RESTART_GATE_SH}`,
  ...
  test("nothing shipped yet: the unit is enabled but not running", async () => {
    const r = await gate({ active: false, envMtime: 1_000_100, unitMtime: 1_000_100 });
  ...
  test("an unreadable start time restarts rather than assuming current", async () => {
    const r = await gate({ mono: "", envMtime: BTIME + 50, unitMtime: BTIME + 50 });
    expect(r.restarted).toBe(true);
  });
});
