...
import type { ValveDeps } from "../../admission-valve";
 ...
 * `build.ts`'s terminal funnel closes over eleven invocation-scoped values, and
 ...
 * row, the transcript, the profile, the ONE verdict) stays in `build.ts`.
 *
 ...
 * does one level down, and `build.ts` routes a failure into its single funnel.
...
export type DeployNamespaceResult =
  ...
  | {
      ...
      failedLabels: string[];
      /** The failure's own sentences, rendered by `build.ts`'s funnel. */
      reason: string[];
    };
