 ...
 * (`build` is in `orphan-guard.ts`'s `OP_COMMANDS`, and `bin/index.ts` reads
 * `process.argv[2]` before any flag parsing). Under `release` this child's ppid
 * is release's own pid, so the guard is inert unless release itself dies — in
 * which case exiting, and thereby dropping `.build.lock`, is the right
 * behaviour.
 */
...
export async function runHermeticBuild(opts: {
  ...
  // from `build.ts` exists to keep true.
