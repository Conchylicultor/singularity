...
import { FATAL_SIGNAL_EXITS, installFatalSignalExit, type FatalSignal } from "./fatal-signals";
    ...
    for (const [sig] of FATAL_SIGNAL_EXITS) {
      ...
      for (const listener of process.listeners(sig)) {
        if (!previous.has(listener)) {
          process.removeListener(sig, listener as (signal: FatalSignal) => void);
        }
      }
    }
  }
  ...
  withInstalled(
    {
      afterInstall: (signals) => {
        seen = signals;
        for (const sig of signals) listenerCounts.set(sig, process.listeners(sig).length);
      },
    },
    () => {
