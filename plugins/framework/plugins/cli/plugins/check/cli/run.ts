...
> = async (checks, opts) => {
  ...
  // `build` via op-runtime/cli/check-subprocess.ts) hands this child its host
  // CPU grant in the environment; `inheritedGrant()` reconstructs it, and we
  // spend those units WITHOUT acquiring host-wide again — no double-acquire, no
  ...
  // runs on an agent branch. See op-runtime/cli/lane.ts.
  ...
  if (marker) {
    ...
    // op-runtime/cli/fatal-signals.ts.
    //
