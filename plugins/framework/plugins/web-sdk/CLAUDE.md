...
- Web:
  - Slots: `Core.Root` ← `apps`, `conversations.model-provider`, `debug.slow-ops`, `infra.health`, `primitives.command-palette`, `primitives.shortcuts`, `reorder.edit-mode`, `reorder.staging`, `reports.crash`, `reports.endpoint-errors`, `reports.mutation-errors`, `shell.global-action-bar`, `shell.toaster`, `ui.theme-engine`, `ui.tokens.font-family.google-fonts`, `Core.Boot` ← `config_v2`, `infra.boot-snapshot`, `ui.theme-engine`, `ui.tweakcn`
- Core:
