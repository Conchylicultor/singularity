...
`key`, `schema`, and keyed-ness (`mode: "keyed"` + `keyOf`) are read from the descriptor; `serverOpts` (`ServerResourceOptions`) carries only the DB half (`loader`, `dependsOn`, `identityTable`, `debounceMs`, `mode` for push-vs-invalidate on a *non*-keyed descriptor). This makes the keyed contract a **single source of truth** — a server that says keyed and a client descriptor that forgot its `keyOf` can no longer drift (that pairing crashed the client with no compile-time signal). `ServerResourceOptions.mode` excludes `"keyed"`, so keyed-ness can only come from the descriptor. The flat one-arg form stays for resources with no shared descriptor (server-only state) — but it is **push/invalidate-only**: a keyed resource CANNOT use the flat form (it has no way to share `keyOf` with the client) and MUST go through the two-arg descriptor form. Requires the server to import the descriptor without a plugin cycle — see the keyed-sync section in `plugins/primitives/plugins/live-state/CLAUDE.md`.
