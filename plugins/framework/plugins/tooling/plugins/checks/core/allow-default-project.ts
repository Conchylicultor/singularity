...
export const allowDefaultProjectInSync: Check = {
  ...
  async run() {
    ...
    // 2. Get the dynamically discovered allowDefaultProject file list
    const adpFiles = discoverAllowDefaultProject(root);

    // 3. Check for overlap (would cause "file already in project" ESLint error)
