...
`MIGRATIONS_PLUGIN_DIR` lives in the same module and is reused by
`check/internal/schema-files-loadable.ts`. Two further copies remain (the
`migrations-in-sync` check and `cli/bin/migrations.ts`); consolidating those
needs a `core` barrel export and is a follow-up.
