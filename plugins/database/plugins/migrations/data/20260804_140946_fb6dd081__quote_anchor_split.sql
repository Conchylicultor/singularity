...
-- migration: 20260804_140946__quote_anchor_split --

-- Quote as a void container anchor — the same split the callout took in
-- `20260729_124840__callout_anchor_split.sql`, for the same reason, and
-- deliberately statement-for-statement identical to it.
--
-- A `quote` row no longer carries `text`: it is now a VOID container whose
-- content is its children, and its former line becomes an ordinary `text` child.
-- Each existing quote R is split into a fresh anchor A (payload `{}` — a quote
-- has no appearance to carry) plus R itself, retyped to `text` and reparented
-- under A.
--
...
-- exactly (they already rendered one indent right of the quote line, inside the
-- bar).
--
...
-- quotes migrating at once. The real rank rides along in `data._migrationRank`
...
SELECT
  ...
  r.parent_id,
  'quote',
  jsonb_build_object('_migrationRank', to_jsonb(r.rank)),
  r.rank || '0',
  true,
  r.deleted_at,
  r.trash_entry_id
FROM page_blocks r
WHERE r.type = 'quote' AND r.data ? 'text';

-- 2. Demote the original row to the quote's first line, under its anchor.
UPDATE page_blocks r
SET parent_id = 'qu-' || r.id,
    rank      = 'a0',
    type      = 'text',
...
--    `type = 'quote'` narrows this to THIS migration's anchors, so it cannot
--    touch a stray `_migrationRank` left by any other split.
UPDATE page_blocks a
SET rank = a.data ->> '_migrationRank',
    data = a.data - '_migrationRank'
WHERE a.type = 'quote' AND a.data ? '_migrationRank';

-- 4. Version-history snapshots embed pre-migration quote blocks verbatim
...
--    restorable; the quote bar is lost, which is confined to already superseded
--    versions and is loud-free.
...
WHERE v.source_id = 'pages'
  AND v.snapshot -> 'blocks' @> '[{"type": "quote"}]';
