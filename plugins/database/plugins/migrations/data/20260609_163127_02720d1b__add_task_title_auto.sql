...
ALTER TABLE "tasks" ADD COLUMN "title_auto" boolean DEFAULT true NOT NULL;--> statement-breakpoint
CREATE VIEW "public"."tasks_v" AS (with "task_completed" as (select "id", EXISTS (
        ...
        ) as "has_blocking_dep" from "tasks" inner join "task_completed" on "task_completed"."id" = "tasks"."id") select "tasks"."id", "tasks"."folder_id", "tasks"."group_id", "tasks"."title", "tasks"."title_auto", "tasks"."description", "tasks"."author", "tasks"."dropped_at", "tasks"."held_at", "tasks"."expanded", "tasks"."rank", "tasks"."created_at", "tasks"."updated_at", 
        CASE
