-- Create the rank_text domain: TEXT with binary (C) collation for fractional-index keys.
-- rankText customType now returns the plain identifier "rank_text" so drizzle-kit
-- emits a simple name instead of the multi-word 'TEXT COLLATE "C"' expression that
-- it incorrectly double-quotes in CREATE TABLE DDL.
CREATE DOMAIN rank_text AS TEXT COLLATE "C";--> statement-breakpoint

-- Drop views that select the rank column before altering its type.
DROP VIEW IF EXISTS "tasks_v";--> statement-breakpoint
DROP VIEW IF EXISTS "agents_v";--> statement-breakpoint

-- Switch existing rank columns from bare TEXT COLLATE "C" to the domain type.
ALTER TABLE "tasks" ALTER COLUMN "rank" SET DATA TYPE rank_text;--> statement-breakpoint
ALTER TABLE "agents" ALTER COLUMN "rank" SET DATA TYPE rank_text;--> statement-breakpoint
ALTER TABLE "quick_prompts" ALTER COLUMN "rank" SET DATA TYPE rank_text;--> statement-breakpoint

-- Recreate agents_v
CREATE VIEW "public"."agents_v" AS (select "id", "parent_id", "name", "description", "prompt", "model", "expanded", "rank", "created_at", "updated_at", ("prompt" IS NULL) as "is_folder" from "agents");--> statement-breakpoint
