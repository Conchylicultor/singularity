...
// read-set, not the base table. View invalidations are FULL (ids = null): a
// view's row identity is not guaranteed to match the base PK. applyDbChange is
...
function routeChange(change: DbChange): void {
  ...
  for (const view of dependentViews(change.table)) {
    applyDbChange({ table: view, op: "U", ids: null });
  }
}
