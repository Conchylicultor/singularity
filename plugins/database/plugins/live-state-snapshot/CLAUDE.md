...
- Server:
  ...
  - Exports (types): `ReadSetShrinkEvent`
  - Exports (values):
...
`onReady` (after the LISTEN is up) does three things in order: force-FULL-recompute
every boot-critical key with no usable persisted read-set; **seed** each persisted
`scopedMembership` alias's in-memory diff base from its durable L2 value
(`seedPersistedSnapshot(key, "{}", value)`) so the first post-boot change is a scoped
refill, not a FULL O(collection) rebuild; then `runCatchUp`. Seeding runs BEFORE
catch-up on purpose — the missed-changes replay then also goes scoped.
