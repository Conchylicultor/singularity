...
// never sweeps the whole table. The extension handle is the source, so the
// projection is its `wireColumns` and the identity is its key `conversationId`
// (the `parent_id` PK); `point.by` IS that identity pk. No orderBy — point sets
// are unordered.
export const conversationNotesResource = windowQueryResource(
  conversationNotesDescriptor,
  {
    from: conversationNotes,
    point: { by: conversationNotes.table.conversationId },
  },
);
