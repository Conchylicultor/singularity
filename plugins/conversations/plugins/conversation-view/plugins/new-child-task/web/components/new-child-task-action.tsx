...
export function NewChildTaskAction() {
  ...
  const submit = async (mode: SubmitMode) => {
    ...
    setSubmitting(mode);
    try {
      // Send the prompt as the task description (server asks Haiku for a
      // short title; we extract paste-image refs so the server can link them
      // to the new task).
      const attachmentIds = extractAttachmentIds(description);
  ...
  return (
    <div className="flex w-80 flex-col gap-2">
      ...
      <PromptEditor
        value={value}
        onChange={onChange}
        onSubmit={() => onSubmit({ kind: "followup" })}
        submitMode="cmd-enter"
        placeholder="Describe the task…"
        autoFocus
        minRows={5}
        maxHeight="14rem"
        namespace="new-child-task"
      />
