import { useResource } from "@plugins/primitives/plugins/live-state/web";
import type { ResourceResult } from "@plugins/primitives/plugins/live-state/web";
import { configV2ConflictsResource, configV2ConflictPathsResource } from "@plugins/config_v2/core";
import type { ConfigV2Conflicts, ConfigV2ConflictPaths } from "@plugins/config_v2/core";

// Raw gateable result — never collapse `pending` into `{}` (that hides
// "still loading" from "genuinely no conflicts"). Callers gate.
// `scopeId` selects which scope's conflicts map to read (undefined = Base).
export function useConflicts(scopeId?: string): ResourceResult<ConfigV2Conflicts> {
  return useResource(configV2ConflictsResource, scopeId ? { scopeId } : {});
}

// storePaths conflicting in the base scope OR any app scope — the aggregate that
// makes a scoped-only conflict visible in the nav badge and rail/sidebar dots
// without opening the descriptor. Gate on `pending` like useConflicts.
export function useConflictPaths(): ResourceResult<ConfigV2ConflictPaths> {
  return useResource(configV2ConflictPathsResource, {});
}
