...
/**
 * Write a full config document into the committed git config layer for a
 * descriptor: `config/<plugin-tree>/<configName>.jsonc`, restamped against the
 * live `<configName>.origin.jsonc` body so the override is born in-sync.
 *
 ...
 *
 * The staged `value` IS the full config document (the field-map object), so it
 * is written verbatim — generalizing the single-`{ items }` reorder case.
 */
export function writeGitLayerOverride(
  ...
  args: {
    ...
    configName: string;
    value: unknown; // full config document
  },
): void {
  const { pluginId, configName, value } = args;
  const hierarchyPath = asPath(asPluginId(pluginId));
  const dir = join(baseDir, "config", hierarchyPath);
  const originPath = join(dir, `${configName}.origin.jsonc`);
  const overridePath = join(dir, `${configName}.jsonc`);

  if (!existsSync(originPath)) {
    throw new Error(
      `Cannot stage a config default: origin file is missing (${originPath}). ` +
  ...
  const str = `// @hash ${hash}\n` + stringifyConfigValue(value as JsonValue) + "\n";
