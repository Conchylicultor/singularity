...
`useOptionalTreeListContext()` + `useSubtreeExpandAll` from the `tree`
primitive instead — the `expand-collapse` sub-plugin is the worked example.
That also fixes a seeding bug the local `useState` had: its lazy initializer
never re-ran, so plugins arriving after first mount stayed collapsed.
