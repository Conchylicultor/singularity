...
/** Prominent "this is an unsaved in-memory draft" banner — no persistence this round. */
function DraftBanner({ name }: { name: string }) {
  return (
    <div className="rounded-md border border-warning/40 bg-warning/10 p-row">
      <Stack gap="2xs">
        <div className="flex items-center gap-xs">
          <MdEdit className="size-3.5 text-warning" />
          <Text variant="label" tone="default">
            Draft — not saved to repo
          </Text>
        </div>
        <Text variant="caption" tone="muted">
          Editing <span className="font-medium">{name}</span> in memory. Toggles
          re-tint the Explorer live; nothing is written to disk.
        </Text>
      </Stack>
    </div>
  );
...
export function CompositionsView() {
  ...
  function selectDraft(manifest: CompositionManifest): void {
    ...
    <Inset pad="md">
      <Stack gap="lg">
        ...
        {isLoading ? (
          ...
          <Text variant="caption" tone="muted">
            No named compositions declared.
          </Text>
        ) : mode === "compare" ? (
          <CompareSection
            manifests={manifests}
      ...
      <Stack gap="sm">
        ...
        <Stack gap="2xs">
          {manifests.map((m) => (
            <Row
              key={m.name}
              selected={active?.name === m.name}
              onClick={() => onSelect(m)}
              actions={
                <Badge size="sm" variant="muted">
                  {m.entryPoints.length} entry · {m.selectedContributors.length}{" "}
                  sel
                </Badge>
              }
