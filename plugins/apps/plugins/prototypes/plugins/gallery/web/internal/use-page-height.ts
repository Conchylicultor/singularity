...
/**
 * How tall a loaded prototype document is, for the `page` frame size.
 *
 ...
 *
 * `null` until a document is ready, and while `enabled` is false — the caller
 * then lays the frame out at `viewportH`.
 */
export function usePageHeight(
  ...
  const measure = () => {
    ...
    setHeight(measurePageHeight(live.frame, live.doc, viewportH));
  };
