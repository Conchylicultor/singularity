...
import { PendingVersionArrows, VersionArrows } from "./version-stepper";

/**
 * The options picker's Version row: the header's `‹ v3 of 7 ›` (the label
 * opens the same version list), plus Back to latest while a past version is
 * shown. For surfaces with no pane header to hold the `VersionStepper` — a
 * presentation — so the version can still be switched there, fullscreen
 * included.
 */
...
function ReadyVersionRow({ history }: { history: PrototypeHistory }) {
  const { model, current, go } = useVersionStepping(history);
