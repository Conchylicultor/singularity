...
**The site's design is its theme, not its components.** The site owns its
theme whole: an `equin` preset for every group the design touches (palette,
chart ramp, type scale, density, shape, font), declared in the shell
(`plugins/shell/web/internal/theme-presets.ts`), and an explicit `default` pin
for every other group — all under `config/ui/tokens/<group>/@app/website/`.
Total on purpose: an unpinned group follows the desktop's runtime theme, which
git cannot show (`tokens:app-theme-pins-total` enforces it). Every component reads
...
The pins live in the **token group** configs because a group's `preset` is what
