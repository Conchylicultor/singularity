...
import { dmdaSourceConfigFields, type DmdaSourceConfig } from "../../core";
 ...
 */
export function readDmdaSourceConfig(config: unknown): DmdaSourceConfig {
  const parsed = schema.safeParse(config);
  if (!parsed.success) {
    throw new NonRetryableError(
      `Invalid config for a "dmda" event source: ${parsed.error.message}`,
      { cause: parsed.error },
    );
  }
  return parsed.data as DmdaSourceConfig;
}
