...
- **`VolumeControl`** (`Sonata.Toolbar`, `components/volume-control.tsx`) — the
  master-volume slider pinned into the Sonata top toolbar. Owns **no** audio.

They communicate through the module-level `audio-store` (`audio-store.ts`,
`useSyncExternalStore`): the control writes `volume`, the engine reads it to
drive master gain; the engine also publishes `status` / `loadError` here.
