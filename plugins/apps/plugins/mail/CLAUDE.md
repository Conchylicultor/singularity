...
- `sync` — the Gmail **sync engine**: paginated backfill, `history.list`
  incremental delta (with a bounded full-resync fallback on `historyId` expiry),
  and a scheduled main-only delta tick. Documents the no-polling exception.
