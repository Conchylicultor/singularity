...
// Binds each mapped MAIL_THREAD_FIELDS id → its physical `mail_threads` column,
...
//
// `labels` binds the jsonb `label_ids` array as `"tags"`, whose filter-sql
// capability compiles the containment operators every mailbox tab's authored
// filter is written in. It is intentionally absent from the sortable set: a
// jsonb array has no keyset-usable order.
export const COLUMN_MAP: FieldColumnMap = {
  ...
  labels: { col: _mailThreads.labelIds, type: "tags" },
