...
export const QueryThreadsBodySchema = z.object({
  ...
  /**
   * The active view's whole filter, lowered by the DataView host (search folded
   * in) — including the mailbox scope, which is an ORDINARY editable rule of the
   * authored view row (`labels contains INBOX`) rather than a server-derived
   * constant. There is no separate scope input: a tab IS its filter. Decoded
   * strictly against `MAIL_THREAD_FILTERABLE`.
   */
