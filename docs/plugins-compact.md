...
- **`config_v2`** [7 sub-plugins] — Reactive useConfig hook for reading typed JSONC config in the browser. Typed JSONC config handles for server plugins.
...
- **`fields`** [33 sub-plugins] — Type-dimension registry: owns the fields.identity slot where each field type registers its identity (token, label, icon, extends, coerce).
...
- **`page`** [10 sub-plugins] — Block-based page editor.
