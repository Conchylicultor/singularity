...
- **`fields`** [2 sub-plugins] — Type-dimension registry: owns the fields.identity slot where each field type registers its identity (token, label, icon, extends, coerce).
...
- **`plugin-meta`** — Plugins about the plugin system itself — browsing, inspecting, and publishing.
  - Plugins:
    ...
    - **`facets`** [33 sub-plugins] — Facet-based plugin metadata extraction and docgen pipeline
    - **`parse-utils`**
