...
- **`apps`** [load-bearing] — App switcher rail. Wraps per-app shells; plugins contribute via Apps.App.
  - Plugins:
    ...
    - **`story`** [10 sub-plugins] — Story Builder — author a page as a block tree and render it through pluggable lenses.
  ...
  - Plugins:
    ...
    - **`tooling`** — Umbrella for build-time tooling: boundary checker, lint rules, checks, guards, codegen
      - Plugins:
        ...
        - **`checks`** [28 sub-plugins] — Check runner and built-in checks for ./singularity check
