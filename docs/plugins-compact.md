...
- **`apps`** [load-bearing] — App switcher rail. Wraps per-app shells; plugins contribute via Apps.App.
  - Plugins:
    ...
    - **`sonata`** [25 sub-plugins] — Sonata — extensible piano and music app.
...
- **`config_v2`** [9 sub-plugins] — Reactive useConfig hook for reading typed JSONC config in the browser. Typed JSONC config handles for server plugins.
...
- **`fields`** [28 sub-plugins] — Type-dimension registry: owns the fields.identity slot where each field type registers its identity (token, label, icon, extends, coerce).
  ...
  - Plugins:
    ...
    - **`tooling`** — Umbrella for build-time tooling: boundary checker, lint rules, checks, guards, codegen
      - Plugins:
        ...
        - **`checks`** [22 sub-plugins] — Check runner and built-in checks for ./singularity check
...
- **`page`** [9 sub-plugins] — Block-based page editor.
...
- **`ui`** — Umbrella for pluggable UI components with switchable visual variants.
  - Plugins:
    ...
    - **`tokens`** [10 sub-plugins] — Umbrella for CSS token group plugins. Contributes global theme presets.
