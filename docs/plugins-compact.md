...
- **`apps`** [load-bearing] — App switcher rail. Wraps per-app shells; plugins contribute via Apps.App.
  - Plugins:
    ...
    - **`sonata`** [22 sub-plugins] — Sonata — extensible piano and music app.
...
- **`page`** [8 sub-plugins] — Block-based page editor.
...
- **`plugin-meta`** — Plugins about the plugin system itself — browsing, inspecting, and publishing.
  - Plugins:
    ...
    - **`facets`** [15 sub-plugins] — Facet-based plugin metadata extraction and docgen pipeline
    - **`parse-utils`**
