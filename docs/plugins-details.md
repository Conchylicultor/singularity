    ...
    - **`workflows`** — Workflows app.
      - Plugins:
        ...
        - **`engine`** — Core engine infrastructure. Defines the Workflows.StepType slot. Core backend infrastructure for the workflows app. Owns DB tables, step executor registry, durable run job, trigger event, HTTP API, and live-state resources.
          ...
          - Cross-plugin:
            - Imported by: `apps/workflows/definitions`, `apps/workflows/executions`, `apps/workflows/steps/branch`
...
- **`primitives`** — Umbrella for cross-cutting client-side primitives used by feature plugins: pane router, tree, live state, networking, editable fields, syntax highlighting, launch buttons.
  - Plugins:
    ...
    - **`css`** — Umbrella for global CSS layout primitives (named-slot rows, grids, clusters, overlays) with the shrink hierarchy baked into one place.
      - Plugins:
        ...
        - **`fill`** — Flexible-cell layout primitive: <Fill axis> is the single grow+shrink cell of a Line/Row (min-w-0 flex-1). The one home for the slack-absorbing, truncation-enabling cell, so a stray flex-1 never strands the grow slot.
          ...
          - Cross-plugin:
            - Imported by: `apps/sonata/library`, `apps/sonata/sources/ultimate-guitar`, `apps/studio/release/release-logs`, `conversations/conversation-view/jsonl-viewer/collapsible-card`
    ...
    - **`editable-field`** — Debounced-autosave field hook with focus tracking, flush-on-blur, and self-echo suppression. Used by task/agent detail forms.
      ...
      - Cross-plugin:
        - Imported by: `apps/pages/page-tree`, `apps/story/shell`, `apps/workflows/definitions`, `conversations/agents`, `conversations/conversation-view/notes`, `conversations/conversations-view/grouped`, `page/code-block`, `page/editor`, `page/math/equation`, `tasks/task-description`, `tasks/task-header`
    ...
    - **`error-boundary`** — Generic React error boundary primitive. Wraps plugin contributions so render errors are contained to one slot, with an ErrorBoundary.Action slot for domain-specific buttons (e.g. crash 'Fix') and a registerBoundaryReporter() hook for opt-in crash reporting.
      ...
      - Cross-plugin:
        - Imported by: `apps/workflows/executions`, `framework/web-core`, `layouts/full-pane`, `layouts/miller`, `reports/crash`, `reports/launch-fix`
