...
- **`database`** — Core database infrastructure. Connection pooling and DB readiness.
  ...
  - Plugins:
    - **`admin`** — Admin operations for the database plugin — fork, backup, drop, list.
      ...
      - Cross-plugin:
        - Imported by: `backup/sources/databases`, `database/change-feed`, `database/db-test-fixture`, `database/fork`, `database/query`, `database/zero/cache-service`, `debug/profiling/ops`, `debug/slow-ops/cluster`, `debug/timeline`, `debug/worktree-cleanup`, `infra/jobs`, `infra/launcher`
