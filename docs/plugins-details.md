    ...
    - **`debug`** — Debug app.
      - Plugins:
        - **`shell`** — App shell for the debug tools. Registers the /debug app entry and defines DebugApp.Sidebar/Toolbar slots.
          ...
          - Cross-plugin:
            - Slot contributors: `backup`, `broadcasts`, `claude-cli-calls`, `events-test`, `live-state-health`, `logs`, `memory`, `profiling`, `queue`, `recover`, `reports`, `worktree-cleanup`
            - Imported by: `backup`, `conversations/recover`, `debug/broadcasts`, `debug/claude-cli-calls`, `debug/live-state-health`, `debug/logs`, `debug/memory`, `debug/profiling`, `debug/queue`, `debug/reports`, `debug/worktree-cleanup`, `infra/events-test`
      ...
      - Plugins:
        ...
        - **`piano-roll`** — Sonata Display: Synthesia-like pitch × time piano roll. Draws notes via its published Projection (time-axis + pitch-plane capabilities), auto-scrolls the time axis to keep the playback cursor in view, and hosts capability-compatible overlays. Server registration of the piano-roll config (Synthesia-style note-name labels).
          - Web:
            ...
            - Uses: `apps/sonata/primitives/inertial-drag.useInertialDrag`, `apps/sonata/primitives/keyboard.isBlackPitch`, `apps/sonata/primitives/keyboard.keyLayout`, `apps/sonata/shell.Sonata`, `apps/sonata/shell.useCursorApi`, `apps/sonata/shell.useSonata`, `apps/sonata/track-mixer.blackKeyColor`, `apps/sonata/track-mixer.useHiddenTrackIds`, `apps/sonata/track-mixer.useTrackColorMap`, `config_v2.ConfigV2`, `config_v2.useConfig`, `config_v2.useSetConfig`, `primitives/log-channels.clientLog`, `primitives/popover.InlinePopover`, `primitives/section-label.SectionLabel`, `primitives/slot-render.renderIsolated`, `primitives/spacing.Stack`, `primitives/text.Text`, `primitives/toggle-chip.ToggleChip`, `primitives/ui-kit.cn`
...
- **`infra`** — Umbrella for cross-cutting server-side primitives used by feature plugins: jobs, events, secrets, mcp, attachments.
  - Plugins:
    ...
    - **`runtime-profiler`**
      ...
      - Core:
        - Exports: Types: `Aggregate`, `ParentBreakdown`, `SlowSpan`, `SlowSpanHandler`, `SpanKind`, `SpanRef`; Values: `getRuntimeProfile`, `installSpanContextRuntime`, `onSlowSpan`, `recordEntrySpan`, `recordSpan`, `resetRuntimeProfile`
...
- **`primitives`** — Umbrella for cross-cutting client-side primitives used by feature plugins: pane router, tree, live state, networking, editable fields, syntax highlighting, launch buttons.
  - Plugins:
    ...
    - **`badge`** — Semantic badge primitive: size × variant chip with a colorClass escape hatch, optional leading icon, and a single theme-derived radius.
      ...
      - Cross-plugin:
        - Imported by: `apps/sonata/sources/midi/folders`, `apps/studio/contributions`, `auth`, `build`, `build/build-info`, `config_v2/settings`, `conversations/conversation-category`, `conversations/conversation-preprompt`, `conversations/conversation-ui/item`, `conversations/conversation-view/allow-monitor`, `conversations/conversation-view/dependent-count`, `conversations/conversation-view/jsonl-viewer`, `conversations/conversation-view/jsonl-viewer/tool-call`, `conversations/conversation-view/jsonl-viewer/tool-call/add-task`, `conversations/conversation-view/jsonl-viewer/tool-call/agent`, `conversations/conversation-view/jsonl-viewer/tool-call/ask-user-question`, `conversations/conversation-view/jsonl-viewer/tool-call/read`, `conversations/conversation-view/jsonl-viewer/tool-call/task-tools`, `conversations/conversation-view/jsonl-viewer/tool-call/workflow`, `conversations/conversation-view/launch-prompts`, `conversations/conversation-view/model`, `conversations/conversation-view/status`, `conversations/conversations-view/grouped`, `conversations/conversations-view/queue`, `conversations/summary`, `debug/broadcasts`, `debug/claude-cli-calls`, `debug/memory`, `debug/profiling/push`, `debug/queue`, `debug/reports`, `debug/worktree-cleanup`, `fields/enum/table`, `infra/events-test`, `plugin-meta/facets/exports/render-contributions`, `plugin-meta/facets/exports/render-detail`, `plugin-meta/facets/structure/render-detail`, `plugin-meta/plugin-view`, `plugin-meta/plugin-view/runtimes`, `primitives/commit-list`, `primitives/file-links`, `review/code-review`, `review/plugin-changes`, `review/plugin-changes/api-changes`, `review/plugin-changes/file-changes`, `shell/notifications`, `tasks/attempt-status`, `tasks/attempt-view`, `tasks/auto-start`, `tasks/task-status`
    ...
    - **`error-boundary`** — Generic React error boundary primitive. Wraps plugin contributions so render errors are contained to one slot, with an ErrorBoundary.Action slot for domain-specific buttons (e.g. crash 'Fix') and a registerBoundaryReporter() hook for opt-in crash reporting.
      ...
      - Cross-plugin:
        ...
        - Imported by: `framework/web-core`, `layouts/full-pane`, `layouts/miller`, `reports`, `reports/launch-fix`
    ...
    - **`relative-time`** — Formats a Date as a human-readable relative string (just now, Nm ago, Nh ago, Nd ago). Exposes formatRelativeTime() and <RelativeTime date={…} />.
      - Cross-plugin:
        - Imported by: `apps/agent-manager/welcome`, `apps/sonata/library`, `apps/sonata/playback-history`, `apps/story/shell`, `build`, `build/build-info`, `conversations/conversation-ui/item`, `conversations/conversation-view/jsonl-viewer`, `debug/claude-cli-calls`, `debug/live-state-health`, `debug/reports`, `fields/date/table`, `plugin-meta/plugin-health`, `shell/notifications`, `tasks/task-header`, `tasks/task-list/recent`
      - Web:
    ...
    - **`tab-id`** — Stable per-tab id (sessionStorage-backed) for crash/notification attribution.
      - Cross-plugin:
        - Imported by: `apps`, `apps/surface-arrangement/desktop`, `debug/reports`, `primitives/live-state`, `reports`, `shell/notifications`
      - Web:
...
- **`reports`** — Reports uncaught browser errors to the server. Records server/frontend crashes and files deduped tasks.
  ...
  - Cross-plugin:
    - Imported by: `conversations`, `conversations/model-provider`, `conversations/runtime-tmux`, `infra/health`, `reports/endpoint-errors`, `reports/noise-rules`, `reports/slow-ops`
  - Plugins:
    ...
    - **`slow-ops`** — Records slow client operations (page load, element appearance) as deduped slow-op reports. Records slow server spans (http/db/loader) as deduped slow-op reports.
      - Web:
...
- **`ui`** — Umbrella for pluggable UI components with switchable visual variants.
  - Plugins:
    ...
    - **`variant-region`** — Factory for pluggable chrome regions with per-app switchable variants. Collapses the config + slot + host + picker + registrations boilerplate into defineVariantRegion (core) and defineVariantRegionWeb (web).
      ...
      - Cross-plugin:
        - Imported by: `apps/app-rail-framing`, `apps/surface-arrangement`, `ui/sidebar-framing`
