      ...
      - Plugins:
        - **`audio`** — Audio playback umbrella for Sonata: engine + instruments.
          - Plugins:
            - **`engine`** — Sonata audio engine: schedules the Score's notes against the Web Audio clock on play, routing each note to its track's resolved instrument, with master volume in the top toolbar.
              - Web:
                - Contributes: `Sonata.Effect` "audio-engine" → `AudioEngine`, `SonataToolbar.End` "volume" → `VolumeControl`
                - Uses: `apps/sonata/shell.getCursorBeat`, `apps/sonata/shell.InstrumentVoices`, `apps/sonata/shell.Sonata`, `apps/sonata/shell.SonataToolbar`, `apps/sonata/shell.useSonata`, `apps/sonata/track-mixer.useMutedTrackIds`, `apps/sonata/track-mixer.useTrackInstrumentMap`, `primitives/icon-button.IconButton`, `primitives/spacing.Stack`
          ...
          - Plugins:
            ...
            - **`key-readout`** — Sonata Section: a current-key readout panel that lights the key's scale notes on a mini keyboard, tracking the playback cursor. Reads the shared Score + cursor from useSonata().
              - Web:
                ...
                - Uses: `apps/sonata/primitives/keyboard.Keyboard`, `apps/sonata/rich/key-mode.saveKeyAutoDetect`, `apps/sonata/shell.setKeyAutoDetect`, `apps/sonata/shell.Sonata`, `apps/sonata/shell.useCursorSelector`, `apps/sonata/shell.useKeyAutoDetect`, `apps/sonata/shell.useSonata`, `primitives/card.Card`, `primitives/spacing.Stack`, `primitives/text.Text`, `primitives/toggle-chip.ToggleChip`
        - **`score`**
          - Cross-plugin:
        ...
        - **`shell`** — App shell for Sonata. Registers the /sonata app entry, owns SonataContext + transport, and defines the Sonata.{Source,Display,Analyzer,Overlay,Instrument,Transport,Section} slots.
          - Web:
            ...
            - Uses: `apps.Apps`, `layouts/full-pane.FullPane`, `primitives/pane-toolbar.definePaneToolbar`, `primitives/slot-render.defineDispatchSlot`, `primitives/slot-render.defineMountSlot`, `primitives/slot-render.defineRenderSlot`, `primitives/text.Text`
            - Exports: Types: `InstrumentVoices`, `ScheduledNote`, `SonataContextValue`, `SonataTransportActions`, `TransportClock`; Values: `getCursorBeat`, `getKeyAutoDetect`, `getSonataTransport`, `publishSonataTransport`, `setKeyAutoDetect`, `Sonata`, `SonataProvider`, `SonataToolbar`, `subscribeCursor`, `subscribeKeyAutoDetect`, `TEMPO_MATH_FLOOR`, `useCursorBeat`, `useCursorSelector`, `useKeyAutoDetect`, `useSonata`
          - Cross-plugin:
