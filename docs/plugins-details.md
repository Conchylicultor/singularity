...
- **`apps`** — App switcher rail. Wraps per-app shells; plugins contribute via Apps.App.
  ...
  - Plugins:
      ...
      - Plugins:
        - **`app-cards`** — Launcher grid of one card per installed app, plus the new-app placeholder.
          - Web:
            ...
            - Uses: `apps.ActiveApp`, `apps.Apps`, `apps.useCurrentAppId`, `apps.useTabs`, `apps/home/shell.Home`, `primitives/data-view.DataView`, `primitives/icon-button.IconButton`
    ...
    - **`surface`** — Per-tab surface: renders every open tab at once positioned by its own placement (docked / floating / solo). Owns the multi-placement body, floating window chrome, geometry store, and the 3-way placement control + Esc-to-exit-solo.
      - Web:
        - Contributes: `Apps.Surface` "Surface" → `SurfaceBody`, `ActionBar.Item` → `ActionBarPlacementControl`, `Shortcuts.Shortcut` "surface.exit-solo (Escape)"
...
- **`plugin-meta`** — Plugins about the plugin system itself — browsing, inspecting, and publishing.
  - Plugins:
    ...
    - **`composition`** — Web hooks + active-composition store for the Studio closure visualization: fetches and deserializes the edge graph once, holds the working draft, and derives membership / inclusion / impact client-side. Serves the classified edge graph + declared composition manifests for the Studio closure visualization.
      ...
      - Cross-plugin:
        - Imported by: `apps/studio/compositions`, `apps/studio/explorer/membership`, `plugin-meta/plugin-view/inclusion`
...
- **`primitives`** — Umbrella for cross-cutting client-side primitives used by feature plugins: pane router, tree, live state, networking, editable fields, syntax highlighting, launch buttons.
  - Plugins:
    ...
    - **`row`** — Generic interactive row primitive (list, menu, nav, tree, and collapsible section-header rows) with a sanctioned home so ad-hoc rounded+padded interactive markup routes through one primitive.
      ...
      - Cross-plugin:
        - Imported by: `active-data/task`, `apps/pages/page-tree`, `apps/sonata/track-mixer`, `apps/studio/compositions`, `build`, `code-explorer`, `code-explorer/file-resolve`, `config_v2/settings`, `conversations/agents`, `conversations/conversation-category`, `conversations/conversation-view/dependencies`, `conversations/conversation-view/jsonl-viewer/tool-call/agent`, `page/editor`, `page/links`, `page/page-link`, `page/turn-into-page`, `plugin-meta/facets/exports/render-detail`, `plugin-meta/plugin-view/sub-plugins`, `primitives/avatar`, `primitives/detail-sections`, `primitives/folder-picker`, `primitives/tree`, `reorder/editor`, `tasks/task-attachments`, `tasks/task-dependencies`, `tasks/task-description`, `tasks/task-events`, `tasks/task-list/recent`, `tasks/task-preprompt`, `ui/theme-engine/theme-customizer`, `ui/tokens/color-palette`, `ui/tokens/density`, `ui/tokens/font-family`, `ui/tokens/shadow`, `ui/tokens/shape`, `ui/tokens/sidebar-palette`, `ui/tokens/type-scale`
    ...
    - **`tab-id`** — Stable per-tab id (sessionStorage-backed) for crash/notification attribution.
      - Cross-plugin:
        - Imported by: `apps`, `apps/surface`, `debug/reports`, `primitives/live-state`, `reports`, `shell/notifications`
      - Web:
