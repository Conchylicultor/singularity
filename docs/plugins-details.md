      ...
      - Plugins:
        - **`shell`** — App shell for the agent manager. Registers the /agents app entry and renders the main Shell layout.
          - Web:
            ...
            - Uses: `apps-core.Apps`, `apps-core.navigate`, `layouts/miller.MillerColumns`, `primitives/app-shell.AppShellLayout`, `primitives/css/text.Text`, `shell.Shell`
          - Core:
      ...
      - Plugins:
        - **`app-cards`** — Launcher grid of one card per installed app, plus the new-app placeholder.
          - Web:
            ...
            - Uses: `apps-core.ActiveApp`, `apps-core.Apps`, `apps-core.useCurrentAppId`, `apps-core.useTabs`, `apps/home/shell.Home`, `primitives/data-view.CreateOption`, `primitives/data-view.DataView`, `primitives/data-view.defineDataView`, `primitives/surface-id.useSurfaceTabId`
      ...
      - Plugins:
        ...
        - **`solo`** — Solo (fullscreen) surface placement — a single tab full-app over everything, with a hover exit button and an Esc shortcut back to the default placement.
          - Web:
            ...
            - Uses: `apps-core.getDefaultPlacement`, `apps-core.getFocusedPlacement`, `apps-core.setFocusedTabPlacement`, `apps-core/surface.Surface`, `primitives/css/pin.Pin`, `primitives/icon-button.IconButton`, `primitives/shortcuts.defineShortcut`
...
- **`primitives`** — Umbrella for cross-cutting client-side primitives used by feature plugins: pane router, tree, live state, networking, editable fields, syntax highlighting, launch buttons.
  - Plugins:
    ...
    - **`css`** — Umbrella for global CSS layout primitives (named-slot rows, grids, clusters, overlays) with the shrink hierarchy baked into one place.
      - Plugins:
        ...
        - **`line`** — Single-line container primitive: <Line> pairs the structural single-line invariant (region-line) with the ambient SingleLineProvider so children never wrap and <Text> leaves truncate. The bare line-container contract composed by Row/Bar and bespoke strips.
          ...
          - Cross-plugin:
            - Imported by: `apps-core`, `apps/studio/release/release-logs`, `conversations/conversation-view/jsonl-viewer/collapsible-card`, `primitives/bar`, `primitives/css/row`, `ui/tab-bar/chip`, `ui/tab-bar/connected`, `ui/tab-bar/underline`
    ...
    - **`sortable-list`** — Generic sortable list primitive with smooth displacement animations. Wraps @dnd-kit/sortable into SortableList + SortableItem components.
      - Cross-plugin:
        - Imported by: `apps-core`, `apps/pages/starred`, `fields/list/config`, `fields/string-list/config`, `layouts/miller`, `primitives/collapsible-wrap`, `primitives/data-view`, `primitives/data-view/view-core`, `reorder`, `reorder/editor`
      - Web:
    ...
    - **`sync-status`** — Per-surface forced sync-status indicator: optimistic/autosave surfaces report {phase,label,retry} via useReportSync; the universal SyncStatusIndicator (mounted once per surface) renders a Google-Keep-style cloud (saving → saved → error+retry). Scoped per surface via scoped-store; tolerates no Provider.
      ...
      - Cross-plugin:
        - Imported by: `apps-core`, `primitives/editable-field`, `primitives/optimistic-mutation`
  ...
  - Plugins:
    ...
    - **`toaster`** — Global toast notifications. Mounts the sonner Toaster and handles Shell.Toast commands.
      - Web:
        ...
        - Uses: `apps-core.useChromeThemeScope`, `primitives/select-scope.ContentScope`, `shell.ShellCommands`, `shell.ToastArgs`, `ui/theme-engine.useColorMode`
  ...
  - Plugins:
    ...
    - **`theme-engine`** — Central settings pane for switching visual variants of pluggable UI components.
      - Web:
        ...
        - Uses: `apps-core.Apps`, `apps-core.useActiveApp`, `apps-core.useCurrentAppId`, `apps-core.useRootThemeScope`, `config_v2.ConfigV2`, `config_v2.useConfig`, `config_v2.useScopeMembership`, `fields/dynamic-enum/config.DynamicEnum`, `primitives/css/ui-kit.appThemeScope`, `primitives/css/ui-kit.themeScopeSelectors`, `primitives/slot-render.defineRenderSlot`
