    ...
    - **`bool`** — Boolean field type: identity only. The data-view cell (check/cross) and filter (yes/no) capabilities live in the plugins/{table,filter} sub-plugins.
      ...
      - Plugins:
        - **`config`** — Boolean field type: config-render capability (checkbox for config-v2.fields.renderer) plus the boolField factory.
          ...
          - Cross-plugin:
            - Imported by: `apps-core/surface/floating`, `apps/mail/mail-core`, `apps/sonata/track-mixer`, `apps/sonata/voicing`, `conversations`, `debug/boot-budget`, `debug/live-state-churn/monitor`, `debug/op-rate`, `debug/paging-probe`, `debug/queue-health`, `debug/read-set-shrink`, `debug/sentinel`, `debug/session-divergence`, `debug/stall-monitor`, `debug/trace/engine`, `infra/duress`, `tasks/tasks-core`
    ...
    - **`int`** — Integer field type: identity only, extends number — reuses number's cell and filter via the extends chain.
      ...
      - Plugins:
        - **`config`** — Integer field type: config-render capability (number stepper for config-v2.fields.renderer) plus the intField factory.
          ...
          - Cross-plugin:
            - Imported by: `apps-core/surface/floating/wallpaper`, `apps/mail/mail-core`, `conversations`, `conversations/summary`, `debug/boot-budget`, `debug/live-state-churn/monitor`, `debug/op-rate`, `debug/paging-probe`, `debug/queue-health`, `debug/sentinel`, `debug/session-divergence`, `debug/slow-ops`, `debug/trace/engine`, `infra/claude-cli`, `infra/duress`, `infra/events`
