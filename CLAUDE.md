...
- `read_page` / `edit_page` / `write_agent_note` — pages, shaped like `Read` / `Edit` / `Write`. **You may only write INSIDE an `<agent-note>` card.** `edit_page` takes any block id, but every block its diff creates, rewrites, moves or deletes must sit in one (a tagless `<agent-note>` in your replacement text mints a card); a page's own prose is read-only — you annotate it, you do not edit it. Before your first page write, `read_page` block **`block-1786031061946-hx4np5`** — the owner's own instructions for these tools, which they keep current.
