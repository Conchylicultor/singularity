...
- **Cross-plugin import grammar.** Only runtime barrels are legal: `@plugins/<name>/{web,server,shared}` for top-level plugins, or `@plugins/<name>/plugins/.../.../{web,server,shared}` for any nesting depth. Forbidden: paths that go *inside* a barrel (`/web/components/`, `/server/internal/`, etc.), workspace-name imports (`@singularity/plugin-shell`), and relative `../` escapes into another plugin's tree.
