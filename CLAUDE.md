...
- **Fix the structural issue, not the specific instance.** When something breaks, ask why it was possible in the first place. A type constraint, a schema change, or an invariant enforced at the boundary prevents the entire class of bug — a targeted fix in one call site leaves the rest exposed. The right fix makes the wrong thing unrepresentable.
------------------------------------
