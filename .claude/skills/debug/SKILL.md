...
- **"Now" vs "old peak"** — the profiler's `recentMaxMs` (rolling ~5-min window) answers "is it slow *right now*"; `maxMs` is the since-boot peak and carries `maxAgeMs`, so an old spike visibly reads as old. For history that survives restarts, the durable **`slow_ops` table** (`query_db`) has real `last_seen_at` / `last_ms`; also surfaced in **Debug → Slow Ops**.
