...
- **No plugin-level theming.** Themes are controlled globally from this folder. Plugins must not define their own theme overrides, color schemes, or mode-specific (`.dark`, `.experimental`) custom property blocks. If a plugin needs a new token, add it here.
