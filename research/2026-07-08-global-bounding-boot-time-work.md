...
2. Firehose check is runtime-registry-based; `./singularity check` is a static process that doesn't load server registrations, so the check can't yet catch a *future* unbounded table that forgets retention. Making it statically load-complete is follow-up (the primitive + the real `_reports` bound are done).
