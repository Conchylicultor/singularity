...
### engine
Core backend infrastructure. Owns 3 DB tables (workflow_definitions with inline steps JSONB, workflow_executions, workflow_execution_steps), the `defineStepExecutor` registry, the `workflows.run` durable job, `userInputSubmitted` trigger event, HTTP API for CRUD on definitions and executions, live-state resources, and the `Workflows.StepType` web slot. Definition steps are inline JSONB (not a separate table) — definitions are read/written as a unit, matching the standard pattern (n8n, AWS Step Functions, Retool). The engine wraps each executor call in `ctx.step()` for replay memoization — `execution_steps` is pure domain state (UI/monitoring), not a replay mechanism. Execution steps snapshot definition fields at creation time so editing a definition never affects running executions.
